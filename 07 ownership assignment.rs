fn main(){
    ...
    let s1: String = getstring(); // 3. s1 is the ownerof hello
    println!("this is s1 : {}",s1);

    let s2:String=String::from("world");     // 4. s2 is the owner of world
    let s3:String= send_get_string(s2);     // 5. transferring the ownership of s2 to send_get_string(s)
    println!("this is s3 : {}",s3);         // 7. s3 is the new owner of world
}
